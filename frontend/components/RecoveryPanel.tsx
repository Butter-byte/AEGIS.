    ...
    <article className={`ra-cand ra-cand-${verdict}`}>
      <div className="ra-cand-head">
        ...
        <span className={`rp-tag ${p.source === "llm" ? "rp-ai" : "rp-neutral"}`}>
          {p.source === "llm" ? "NVIDIA NEMOTRON" : "HEURISTIC"}
        </span>
      </div>
...
function buildExplanation(result: RecoveryRunResult): string[] {
  ...
  if (applied) {
    ...
    lines.push(
      `To fix this, the system used the "${plan.strategy_label}" strategy (generated by ${plan.source === "llm" ? "NVIDIA Nemotron AI" : "the built-in heuristic engine"}).`,
    );
      ...
      <div className="rp-block-head">
        ...
        {source && (
          <span className={`rp-tag ${source === "llm" ? "rp-ai" : "rp-neutral"}`}>
            {source === "llm" ? "NVIDIA NEMOTRON" : "HEURISTIC"}
          </span>
        )}
      </div>
